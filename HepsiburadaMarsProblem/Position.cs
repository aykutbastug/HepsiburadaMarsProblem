...
namespace HepsiburadaMarsProblem
{
            ...
            foreach (var move in moves)
            {
                switch (move)
                {
                    ...
                    case 'R':
                        ...
                        break;
                    default:
                        Console.WriteLine($"Geçersiz karakter girdiniz : {move}");
                        break;
                }

                if (this.X < 0 || this.X > maxPoints[0] || this.Y < 0 || this.Y > maxPoints[1])
                {
                    throw new Exception($"Hata! Konum sınırların üstünde olamaz (0 , 0) ({maxPoints[0]} , {maxPoints[1]})");
                }
            }
        }
    }
